...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, _, i) => acc + str.charCodeAt(i), 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((acc, cur) => acc + +vowels.includes(cur.toLowerCase()), 0);
}
...
function isPalindrome(str) {
  const testStr = str
    .toLowerCase()
    .split('')
    .filter((sym) => sym.match(/[a-z]/i))
    .join();
  return testStr.split('').reverse().join('').includes(testStr);
}
...
function findLongestWord(sentence) {
  ...
  return arr.filter((word) => maxLength === word.length).at(0);
}
  ...
  return str
    ...
    .map((word) =>
      word
        .split('')
        .map((letter) =>
          isUpperCase(letter) ? letter.toLowerCase() : letter.toUpperCase()
        )
        .join('')
    )
    .join(' ');
...
function encodeToRot13(str) {
  return str
    ...
    .map((letter) => {
      ...
      if ((code > 90 && code < 97) || letter === '' || code < 65) {
        return letter;
      }
      if (code > 64 && code < 91) {
        if (code + 13 < 91) {
        ...
        return String.fromCharCode(code + 13 - 26);
      }
      if (code > 96 && code < 123) {
        if (code + 13 < 123) {
          return String.fromCharCode(code + 13);
